---
title: '02: Create the lab alert, incident with a custom detection rule, and detect password spray attempts'
layout: default
nav_order: 2
...
## Task 02: Create the lab alert, incident with a custom detection rule, and detect password spray attempts

We'll detect a password spray (>=10 failed attempts across >=5 accounts from one IP within an hour) and let Defender create an alert that rolls into an incident. 

1. Clear the existing query in Advanced Hunting.

1. Copy the following KQL into the query window, select the entire query, then select **Run query**.  

    <details markdown='block'>
    <summary>
    Expand here to copy the KQLQuery-2
    </summary>
    ...
    let FailAad =
        ...
        | where ErrorCode != 0
        | project Timestamp, IPAddress, AccountUpn, SrcTable = "AAD";

    let FailId =
        IdentityLogonEvents_all
        | where Timestamp between (Start .. End)
        | where ActionType =~ "LogonFailed" or isempty(FailureReason) == false
        | project Timestamp, IPAddress, AccountUpn, SrcTable = "ID";

    let Failures = union FailAad, FailId;
    ...
    | summarize SprayAttempts = count(), FirstSeen = min(Timestamp), LastSeen = max (Timestamp) by IPAddress, AccountUpn
    | project Timestamp = LastSeen, IPAddress, AccountUpn, SprayAttempts
    ```
    </details>
    ...
    ![MonitorInvestigate-2.png](../../media/MonitorInvestigate-2.png)

    {: .note }
    > The results will show the password spray attempts.